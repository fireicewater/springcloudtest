...
//				String attachment = client.getAccount(recipient.getAccountName());
				JSONObject jsonObject = restTemplate.getForEntity("", JSONObject.class, recipient.getAccountName()).getBody();
//				emailService.send(type, recipient, attachment);
				emailService.send(type, recipient, jsonObject.toJSONString());
